...
RG = (function() {
  ...
  let getCollationData, getUnitData, recollate, runCollation, showVerseCollation, allRuleStacksEmpty, doSelectAll;
  ...
  let _calculateLacWits, _hasRuleApplied, _getDisplayClasses, _getToken, _getWordTokenForWitness,
      ...
      _scheduleSelectedRulesDeletion, _addFooterFunctions, _highlightAddedWitness, _getRulesForDisplay, _scheduleAllRuleDeletion;
...
//PR added this so we can select all the regularizations at this point
  ...
  getUnitData = function(data, id, start, end, options) {
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      rowList.push(rowId);
      if (i === 0) { //PR added icon to 
//        cells.push('<tr><td class="redips-mark" colspan="MX_LN"><span id="toggle_variant_' + id + '" class="triangle">&#9650;</span></td></tr>');
   		   cells.push('<tr><td class="mark" colspan="MX_LN"><img onclick="RG.doSelectAll(this)" id="'+i+'" class="selectall" src="'+staticUrl+'CE_core/images/checkall.png" height="16px"><span id="toggle_variant_' + id + '" class="triangle">&#9650;</span></td></tr>');
      }
      classes = [];
      if (i === 0) {
        classes.push('top');
...
//     console.log("witness list "+ CL.dataSettings.witness_list)
 ...
 //end PR additions
      ...
      createFunction = function() {
        ...
        CL.services.getUserInfo(function(user) {
          ...
          for (var i=0; i<selreg.length; i++) {
		 	_rules[selreg[i].objOld.id] = _createRule(data, user, selreg[i].original_text, normalisedText, selreg[i].unit, selreg[i].reading, selreg[i].word, selreg[i].witnesses);
  ...
  _makeMenu = function(menuName) {
    if (menuName === 'regularised') {  //changed by PR
  ...
  _addContextMenuHandlers = function() {
    ...
    if (document.getElementById('delete_all_reg')) {
	  ...
	  $('#delete_all_reg').on('click.dar_c', function(event) {_scheduleAllRuleDeletion();});
	  $('#delete_all_reg').on('mouseover.dar_mo', function(event) {CL.hideTooltip();});
	}
  ...
  if (testing) {
    return {
      ...
      _scheduleAllRuleDeletion: _scheduleAllRuleDeletion
