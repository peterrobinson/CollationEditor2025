...
CL = (function() {
  ...
  let setServiceProvider, expandFillPageClients, getHeaderHtml, addUnitAndReadingIds,
      ...
      getSpecifiedAncestor, hideTooltip, addHoverEvents,addShiftkeyEvents, markReading, showSplitWitnessMenu, markStandoffReading,
 ...
 addShiftkeyEvents = function (row, witnesses) {
  ...
  _prepareCollation = function(output) {
    ...
    if (context && CL.dataSettings.base_text !== 'none') {
      ...
      console.log("moving to collate "+context+" debug "+ CL.debug)
  ...
  _findSaved = function(context) {
  	console.log("loading _findSaved")
    spinner.showLoadingOverlay();
  ...
  _showSavedVersions = function(data, projectWitnesses, context) {
    ...
    if (data.length > 0) {
      for (let i = 0; i < data.length; i += 1) {
        ...
        //PR adds -- mu collations do not have a created time. Is that because thos come from the file system..
  //      console.log("assigning date string")
        if (!(!isNaN(date) && date.toString() !== 'Invalid Date')) {
        	date=new Date();
        	if (date.getMinutes() < 10) { minutes = '0' + date.getMinutes();} else { minutes = String(date.getMinutes());}
...
//    console.log("save divs needed")
  ...
  _addToSavedCollation = function(id) {
  	console.log("in addtosavecollation")
    var data, collId;
    if (id === undefined) {
  ...
  _loadSavedCollation = function(id) {
    var data, collId;
    console.log("in _loadSavedCollation")
    CL.isDirty = false;
    spinner.showLoadingOverlay();
    if (id === undefined) {
      data = cforms.serialiseForm('saved_collation_form');
  ...
  if (testing) {
    return {
      ...
      addShiftkeyEvents: addShiftkeyEvents,
